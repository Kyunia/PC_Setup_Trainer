...
export function resolvePerSaveTargetLines({targetLines,clear}={}){
  ...
  if(!Number.isInteger(resolved)||resolved<2||resolved>4){
    throw new PerSaveMinimalsInputError(`per-save minimals targetLines must be 2, 3, or 4; got ${resolved}`);
  }
  return resolved;
}
...
export function calculatePerSaveMinimals({sourceFumen,pattern,solver,useHold=true,targetLines,clear}){
  ...
  return{
    ...calculatePerSaveMinimalsFromBoard({board:geometry.board,queues,solver,useHold}),
